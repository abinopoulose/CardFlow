      ...
      {previewDataset.map((currentData, index) => (
        <div key={index} className="flex flex-col items-center shrink-0 group relative">
          ...
          <button 
            ...
            className="absolute top-8 right-2 z-10 p-2 bg-indigo-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-lg hover:bg-indigo-700 pointer-events-auto"
      ...
      {fullScreenData && createPortal(
        <div className="fixed inset-0 z-[100] bg-gray-900/90 backdrop-blur-sm flex items-center justify-center p-8">
          ...
          <div className="w-full h-full flex items-center justify-center cursor-grab active:cursor-grabbing">
            <TransformWrapper
              initialScale={Math.min(1, (window.innerWidth - 100) / width, (window.innerHeight - 100) / height)}
              minScale={0.1}
              maxScale={5}
              centerOnInit
              wheel={{ step: 0.05 }}
            ...
            {fields.map((field) => {
              const rawVal = fullScreenData[field.headerKey];
              const isImage = field.type === 'image';
              const isShape = field.type === 'shape';
              const isQrCode = field.type === 'qrcode';
              const isBarcode = field.type === 'barcode';
              ...
              if (isImage) {
                if (field.isStatic) {
                  ...
                  const idNumber = fullScreenData['id_number'];
                  if (idNumber && photosMap[field.headerKey]?.[String(idNumber)]) {
                    val = photosMap[field.headerKey][String(idNumber)];
                  }
                }
              ...
              return (
                ...
                >
                  {field.type === 'shape' ? (
                    ...
                    >
                      {(() => {
                        let txt = field.isStatic ? (field.staticText || '') : (fullScreenData[field.headerKey] !== undefined && fullScreenData[field.headerKey] !== null ? String(fullScreenData[field.headerKey]) : '');
                        if (field.textTransform === 'sentence' && txt) {
                          txt = txt.charAt(0).toUpperCase() + txt.slice(1).toLowerCase();
                        }
                        return txt;
